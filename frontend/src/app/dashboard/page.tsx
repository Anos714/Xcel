    ...
    <div className="mx-auto w-full max-w-7xl space-y-8">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            Monitor publishing, automation, and ideas from one calm workspace.
          </p>
        </div>
        <Link href="/tweets" className={cn(buttonVariants(), "gap-2")}>
          <Send /> Create a tweet
        </Link>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
            ...
            {data?.recentTweets?.length ? (
              data.recentTweets.map((tweet) => (
                ...
                >
                  ...
                  <Badge
                    variant={
                      tweet.status === "posted" ? "default" : "secondary"
                    }
                  >
                    {tweet.status}
                  </Badge>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {data?.upcomingTweets?.length ? (
              data.upcomingTweets.map((tweet) => (
                <div
                  key={tweet.id}
              ...
              <p className="text-sm text-muted-foreground">
                Nothing scheduled. Your next post can go here.
              </p>
            )}
          </CardContent>
        </Card>
