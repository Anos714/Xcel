...
export const createQuery = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    return sendResponse(res, 201, "Query created successfully", response);
  },
);

export const getQueries = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const response = await queryService.getQueries();
    if (!response) {
      return sendResponse(res, 404, "Failed to fetch queries");
    }

    return sendResponse(res, 200, "Queries fetched successfully", response);
  },
);

export const updateQuery = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!response) {
      return sendResponse(res, 404, "Failed to update query");
    }

    return sendResponse(res, 200, "Query updated successfully", response);
  },
);

export const deleteQuery = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const paramResult = paramsSchema.safeParse(req.params);
    ...
    if (!response) {
      return sendResponse(res, 404, "Failed to delete query");
    }

    return sendResponse(res, 200, "Query deleted successfully", response);
  },
);
